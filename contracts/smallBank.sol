...
contract smallBank is ERC20("ABCBank", "ABCBank") {
    ...
    function lend(uint256 amount) external {
        ...
        lenders[lendingID] = lendingData(msg.sender,amount+interest, block.timestamp + lockingPeriod,false);
    ...
    function borrow(uint amount) external {
        checkCollatoral();
        uint interest = (amount*interestRate)/100;
        borrowers[borrowingID] = borrowingData(msg.sender,amount+interest, block.timestamp + deadline,false);
    ...
    function withDrawFD(uint id) external{
        ...
        require(transferFrom(address(this), msg.sender, lenders[id].amount),"Unable to withdraw FD");
    ...
    function payback(uint id, uint amount) external {
        require(amount>=borrowers[id].amount,"Not sufficient payable amount");
